        ...
        <div className='w-full relative'>
            <div className='relative w-full'>
                ...
                {forgotPassword && <Link href="/auth/forgot-password" className='text-xs text-white/80 absolute underline -bottom-2 right-0 translate-y-full'>{t('passwordForgotten')}</Link>}
                {
                    type === "password" && (
                        <>
                            <LuEyeClosed onClick={() => setShowPassword(prev => !prev)} className={`absolute top-1/2 -translate-y-1/2 right-4 text-zinc-600 origin-center cursor-pointer transition-all duration-150 ${!showPassword ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`} />
                            <LuEye onClick={() => setShowPassword(prev => !prev)} className={`absolute top-1/2 -translate-y-1/2 right-4 text-zinc-600 origin-center cursor-pointer transition-all duration-150 ${showPassword ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`} />
                        </>
                    )
                }
            </div>
