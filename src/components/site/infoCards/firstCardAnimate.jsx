...
export default function FirstCardAnimate() {
    ...
    return (
        <article ref={containerRef} className='w-full max-w-[43%] relative'>
            <Image src="/images/card-bg-2.svg" alt="info card" width={600} height={400} className="absolute z-10 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-full h-full min-h-[600px] min-w-[650px] object-contain object-center opacity-70" />
            <div className='relative w-fit h-fit mx-auto z-10 bg-white/95 rounded-xl p-14 grid grid-cols-3 gap-y-8 gap-x-8 shadow-sm'>
                {socialIcons.map((icon, index) => {
                    const Icon = icon.icon;
                    return (
                        <div key={index} className='social-icon flex items-center justify-center w-fit h-fit bg-deep-teal rounded-full p-4 shadow-xl' style={{ filter: 'blur(4px)' }}>
